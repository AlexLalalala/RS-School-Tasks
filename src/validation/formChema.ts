...
export const  formSchema = yup.object({
  name: yup.string().required(requiredMessageFactory('Name')).matches(/^[A-Z]/, 'Name must start with an uppercase letter'),

  age: yup.number().required(requiredMessageFactory('Age')).min(0, 'Age must be positive'),

  email: yup.string().required(requiredMessageFactory('Email')).email('Invalid Email'),
  ...
  password: yup.string()
    ...
    .matches(/[^a-zA-Z0-9]/, symbolRequirementMessageFactory('special character')),
  ...
  file: yup.mixed<FileList>().required(requiredMessageFactory('Image'))
    .test('filePresent', 'Image is required', 
      (value)=>{
        return value instanceof FileList && value.length > 0
      })
    .test('fileType', 'Must be .png or .jpeg', 
      (value)=>{
        if (value instanceof FileList && value.length === 0)
          return true
        return ALLOWED_TYPES.includes(value[0].type)
      })
    .test('fileSize', 'Must be under 5Mb', 
      (value)=>{
        if (value instanceof FileList && value.length === 0)
          return true
